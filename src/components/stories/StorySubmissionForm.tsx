    ...
    <div className="space-y-6">
      ...
      <div className="bg-blue-50 p-6 rounded-2xl border border-blue-100 flex items-center justify-between">
        <div className="flex items-center gap-4">
          <div className={`w-12 h-12 rounded-full flex items-center justify-center text-white transition-all ${isRecording ? 'bg-red-500 animate-pulse' : 'bg-primary'}`}>
            {isRecording ? '⏹️' : '🎙️'}
          </div>
          <div>
            <h4 className="font-bold text-blue-900">Voice Narrative Assistant</h4>
            <p className="text-xs text-blue-700">Record your story and let AI transcribe it automatically.</p>
          </div>
        </div>
        <button
          type="button"
          ...
          className={`px-6 py-2 rounded-full font-bold text-sm transition-all ${
            isRecording ? 'bg-red-500 text-white' : 'bg-white text-primary border border-blue-200 hover:bg-blue-100'
          }`}
        >
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="bg-white p-8 rounded-2xl border border-gray-200 shadow-sm space-y-6">
        ...
        <button
          ...
          className={`w-full py-4 rounded-xl font-bold text-white shadow-lg transition-all ${
            isSubmitting ? 'bg-gray-400' : 'bg-primary hover:bg-primary-dark transform hover:-translate-y-0.5'
          }`}
        >
          {isSubmitting ? 'Saving to Archive...' : 'Preserve Story'}
        </button>
      </form>
    </div>
  )
